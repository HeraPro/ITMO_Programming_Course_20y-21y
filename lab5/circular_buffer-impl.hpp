...
//        alloc_traits::construct(alloc_, head + i);
//        //head[i] = 0;
//    }
...
CircularBuffer<Data, Alloc>::CircularBuffer(const CircularBuffer &buffer): //fixed
        ...
        alloc_(alloc_traits::select_on_container_copy_construction(buffer.alloc_)){
    //alloc_alloc_traits::select_on_container_copy_construction(alloc_);
    head = alloc_traits::allocate(alloc_, capacity_);
    start_ = head;
    for (size_t i = 0; i < size_; ++i) {
        alloc_traits::construct(alloc_, head + i, buffer.operator[](i));
        //operator[](i) = buffer[i];
    }
}
...
void CircularBuffer<Data, Alloc>::push_back(const Data &element) { //fixed
    ...
    alloc_traits::destroy(alloc_, &back());
...
template<typename Data, typename Alloc>
void CircularBuffer<Data, Alloc>::push_front(const Data &element) { //fix
    if (size_ != capacity_) {
        ++size_;
    }
    ...
    alloc_traits::destroy(alloc_, &front());
    alloc_traits::construct(alloc_, &front(), element);
}

template<typename Data, typename Alloc>
void CircularBuffer<Data, Alloc>::pop_front() {
